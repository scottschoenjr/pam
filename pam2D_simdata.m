...
tic;
sourceFile = ...
    '../data/waterdata2lambda_3Bubbles_z.mat';
...
tic
p=rfasa';       %rf data for 128 channels - CC moved outside loop
aa1=(fft(p));  % fft of whole zero padded data - CC moved outside loop
for mm=1:ss(1)
    fc=frqax(fbins(mm));
    w=2*pi*fc;
    [zz, ff]=(min(abs(fk-fc)));% ff the data at fc
    P=aa1(ff+1,:);% the complex data at fc for each channel - CC based on single FFT outside loop
    P=fftshift(fft(P)); %convert P into K-space
    % image reconstruction algorithm
    k=2*pi*(-floor(length(x)/2):ceil(length(x)/2)-1)/(x(2)-x(1))/length(x);% wavenumber
    asa=zeros(length(z),pchannels);%final image
    ...
    asa = ifft(ifftshift(Pv.*exp(1i.*zv'.* sqrt(w.^2./c.^2-kv.^2)),2),[],2);
